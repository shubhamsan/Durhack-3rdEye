...
3rd Eye is an assistive-vision prototype designed to give visually-impaired users a remote “second camera” that moves automatically based on their head direction.

The system streams live video from a Raspberry Pi to a PC while using real-time face landmark tracking (Mediapipe) to rotate a servo-mounted camera.
...
Blind and visually-impaired people often struggle to explore their surroundings independently.
3rd Eye acts as a remote controllable viewpoint:
...
No Node.js, web, or browser components were used in this implementation.
...
The Pi continuously streams frames → PC,
while the PC continuously sends angles → Pi.
